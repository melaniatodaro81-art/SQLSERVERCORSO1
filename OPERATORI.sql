...
-- RECUPERA TUTTE LE COLONNE NELLA TABELLA STUDENTI CHE INIZIA XS E SONO FEMMEINE- 
 --DOVE LIKE STA A DEFINIRE UN CI STA E IN QUESTO CASE CI STA ALL'INIZIO O CI STA ALLA FINE

SELECT * FROM Studenti where Nome like '%e' and Genere = 'M';

 -- VISUALIZZARE TUTTE LE RIIGHE DOVE IL NOME FINIESCE CON E IL GENERE Eè MASCHILE
...
select *
from Studenti 
where Nome = 'Alice', and Nome ='Melania', Nome = 'Giulia';

select *
from Studenti 
where Nome IN ('Alice', 'Melania', 'Giulia');

--RESTITUISCE  LE RIGHE SENZA MAIL DELLA SELEZIONE
SELECT DISTINCT *
From Studenti 
where NOME IN ('Alice', 'Melania', 'Giulia') 
